...
export const api = axios.create({
  ...
  headers: {
    ...
    // Prevent ngrok's free-tier browser warning page from
    // being returned for API requests.
...
// ==========================================================
//
// Natural-language interface to the SmartPark AI backend.
//
// Backend endpoint:
//
// POST /ai/chat
//
// The chatbot may optionally provide the user's browser
// location for location-aware questions such as:
//
// - "What parking is near me?"
// - "What is the nearest parking facility?"
// - "Which parking facility is closest to me?"
//
// Location is optional because many SmartPark AI questions
// do not require the user's current location.
// ==========================================================
...
export interface AIChatRequest {
  ...
  /**
   * Browser-provided latitude.
   *
   * The user does NOT need to type this.
   * SmartParkChatbot obtains it through the browser
   * Geolocation API when permission is granted.
   */
  latitude?: number | null;

  /**
   * Browser-provided longitude.
   *
   * The user does NOT need to type this.
   * SmartParkChatbot obtains it through the browser
   * Geolocation API when permission is granted.
  ...
  /**
   * OpenAI Responses API response ID used to preserve
   * conversational continuity across chat requests.
   */
...
export interface AIChatResponse {
  /**
   * Natural-language response generated by SmartPark AI.
   */
  message: string;

  /**
   * OpenAI Responses API response ID for the current turn.
   */
  response_id: string;
}
...
export const aiApi = {
  /**
   * Send a natural-language message to SmartPark AI.
   *
   * Backend endpoint:
   *
   * POST /ai/chat
   ...
   *
   * "Tell me about Two Rivers Mall"
   *
   * "What is the nearest parking facility to me?"
   *
   * "How many EV charging bays are available at Two Rivers Mall?"
   */
   ...
   * Backend endpoint:
   *
   * POST /ai/chat/attachment
   *
   * The existing JSON chat contract remains unchanged.
   */
  chatWithAttachment: async (
    ...
    if (payload.previous_response_id) {
      formData.append("previous_response_id", payload.previous_response_id);
    }
